...
// Route: '/profile' (GET)
// This route shows the user's playlists
app.get('/profile', async (req, res) => {
  try {
      // Step 1: Query the 'playlists' table to get all playlists
      const result = await db.query('SELECT * FROM playlists');
      const playlists = result.rows;  // Get the playlists for the user
      
      // Step 2: Render the 'profile.ejs' page and pass the playlists to it
      // The user will see their playlists on the profile page
      res.render('profile', { playlists });
  } catch (err) {
      // If an error occurs
      console.error('Error fetching playlists:', err);
